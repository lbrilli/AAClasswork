...
def prolific_actors
  ...
  HAVING COUNT(movie_id) >= 15 AND castings.ord = 1
...
def films_by_cast_size
  ...
  execute(<<-SQL)
  SELECT movies.title, COUNT(movie_id)
  FROM movies
  JOIN castings ON castings.movie_id = movies.id
  GROUP BY movies.title, yr
  HAVING movies.yr = 1978
  ORDER BY COUNT(movie_id) DESC, title ASC
  SQL
end
